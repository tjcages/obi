...
import { cn } from "../../lib";
import { ScriptBlock } from "./_script-block";
import { ResultCards } from "./_result-cards";
...
export function ToolInvocation({ invocation }: ToolInvocationProps) {
  ...
  return (
    <div
      className={cn(
        "rounded-lg border border-border-100 bg-background-200/80 w-full min-w-0 max-w-full overflow-hidden",
        state === "input-available" && "border-l-4 border-l-green-500 animate-pulse",
        (state === "output-available" || state === "output-error") && "border-l-4 border-l-green-500/50",
      )}
    >
      <div className="flex items-center gap-2 px-4 py-2 text-base text-foreground-200">
        <PlayIcon />
        <span>{displayIntent}</span>
      </div>
      <ScriptBlock code={displayCode} isStreaming={state === "input-streaming"} />
      {state === "input-streaming" && (
        <div className="px-4 py-2 text-sm text-foreground-300">Writing script...</div>
      )}
